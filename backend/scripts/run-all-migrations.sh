...
for migration in migrate:consolidated migrate:video-url migrate:fileurl-goodies migrate:access-level migrate:access-level-courses migrate:bank-transfer-payments migrate:stripe-payments migrate:te-natiraa-registrations migrate:te-natiraa-events migrate:legacy-verifications migrate:polls migrate:conversations-messages migrate:is-certified migrate:blog-status migrate:kikiri migrate:kikiri-tables migrate:kikiri-config migrate:kikiri-betting-duration migrate:kikiri-post-resolution-delay migrate:bingo migrate:bingo-fix-roundid migrate:bingo-grid-price migrate:bingo-winner-ids migrate:jiji migrate:game-period-winners migrate:bingo-ended-at; do
  echo ">>> $migration"
