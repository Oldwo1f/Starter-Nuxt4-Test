...
const handleFileSelect = (event: Event) => {
  ...
  if (props.currentImagesCount >= props.maxImages) {
    toast.add({
      title: 'Limite atteinte',
      description: `Maximum ${props.maxImages} images autorisées`,
      color: 'warning',
    })
    return
  }
...
// Calculer la taille de sortie (ratio 4/3)
const calculateOutputSize = (canvas: HTMLCanvasElement) => {
  const maxWidth = 1920
  const aspectRatio = 4 / 3
  
  // Calculer la largeur et hauteur en gardant le ratio 4/3
  let width = canvas.width
  let height = canvas.height
  
  // Ajuster pour respecter le ratio 4/3
  const currentRatio = width / height
  if (currentRatio > aspectRatio) {
    // L'image est plus large que 4/3, ajuster la hauteur
    height = width / aspectRatio
  } else {
    // L'image est plus haute que 4/3, ajuster la largeur
...
const confirmCrop = async () => {
  ...
  try {
    ...
    outputCanvas.toBlob((blob) => {
      ...
      const file = new File([blob], `blog-image-${Date.now()}.jpg`, { type: 'image/jpeg' })
...
<template>
  <div>
    ...
    <div v-if="!showCropper">
      ...
      >
        ...
        <span class="mt-1 text-xs text-white/60">
          Formats : JPEG, PNG, WebP (max 10MB) • Ratio 4/3 requis
        </span>
      </label>
    </div>
    ...
    <UModal 
      ...
      description="Ajustez l'image pour un format 4/3. L'image sera redimensionnée si nécessaire."
